...
toCheckpoint solution preview =
    let
        direct =
            Dict.foldr (addBound solution) Dict.empty preview.direct

        indirect =
            Dict.foldr (addBound solution) Dict.empty preview.indirect
    in
            ...
            case getCheckpoint model.status of
                ...
                ( _, [] ) ->
                    case model.past of
                        ...
                        latest :: previous ->
                            await (toInitialCheckpoint model.origin)
                                previous
...
await checkpoint changes model =
    let
        ...
        preview =
            toPreview model.origin checkpoint changes
    in
    ( { model
...
viewUndoRedo status past future =
    ...
    div []
        [ if hasNoPast then
            inactiveButton undoIcon

          else
            activeButton Undo undoIcon
        , if hasNoFuture then
            inactiveButton redoIcon

          else
            activeButton Redo redoIcon
        ]
...
viewRow pkg info =
    case info of
        ...
        RowOld old new ->
            if old == new then
                viewRowHelp pkg (text "") (text "") (viewVersion "#cccccc" new)

            else
                viewRowHelp pkg (viewVersion "#cccccc" old) (viewArrow "#cccccc") (viewVersion "black" new)

        RowOldGuess old new ->
            if old == new then
                viewRowHelp pkg (text "") (text "") (viewVersion "#eeeeee" new)

            else
                viewRowHelp pkg (viewVersion "#eeeeee" old) (viewArrow "#eeeeee") (viewVersion "#eeeeee" new)
...
updateSearch registry msg search =
    case msg of
        ...
        SDown ->
            let
                numMatches =
                    List.length (getBestMatches search.query registry)
        ...
        SEnter ->
            case search.focus of
                ...
                Just 0 ->
                    if Dict.member search.query registry then
                        SAdd search.query
...
insert limit name dist bestMatches =
    if limit <= 0 then
        ...
        case bestMatches of
            ...
            (( bestDist, _ ) as best) :: worseMatches ->
...
encodeConstraint bounds =
    case bounds of
        POld (Version x y z) _ oldBounds ->
            case oldBounds of
                OLocked ->
                    E.string <| v2s x y z ++ " <= v < " ++ v2s x y (z + 1)

                OPatch ->
                    E.string <| v2s x y z ++ " <= v < " ++ v2s x y max16

                OMinor ->
                    E.string <| v2s x y z ++ " <= v < " ++ v2s x max16 0

                OMajor ->
                    E.string <| v2s x y z ++ " <= v < " ++ v2s max16 0 0
